...
import {authenticatedAs, deleteTask} from '../../../testing/todoListApi';
...
describe('Delete task E2E', () => {
    ...
    scenario('deletes an added task', async () => {
        const id = `deletetask-e2e-${Date.now()}`;

        await given(api.addTask(id, {name: 'Milk'}))
            .when(api.deleteTask(id))
            .then([expectResponse(201, {body: {ok: true}})]);
    });

    scenario('deletes the same task twice', async () => {
        const id = `deletetask-e2e-twice-${Date.now()}`;

        await given(api.addTask(id, {name: 'Milk'}), api.deleteTask(id))
            .when(api.deleteTask(id))
            .then([expectResponse(201, {body: {ok: true}})]);
    });
