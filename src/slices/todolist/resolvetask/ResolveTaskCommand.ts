...
export type ResolveTaskCommand = Command<'ResolveTask', {
    id: string;
}, {
...
export const handleResolveTask = async (id: string, command: ResolveTaskCommand) => {
    ...
    const result = await ResolveTaskCommandHandler(
        eventStore,
        id,
        (state: ResolveTaskState) => decide(command, state),
    );
    return {
